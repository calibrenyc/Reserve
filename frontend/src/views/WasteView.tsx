    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleCreate} className="bg-zinc-900 border border-zinc-700 p-4 rounded-xl flex items-center gap-3">
        ...
        <input
          ...
          className="bg-zinc-950 border border-zinc-700 rounded-lg px-3.5 py-2.5 text-sm text-zinc-100 w-36 focus:border-emerald-500 focus:outline-none"
        />
        ...
        <button
          type="submit"
          className="px-4 py-2.5 bg-rose-600 hover:bg-rose-500 text-white font-semibold text-sm rounded-lg flex items-center space-x-1.5 cursor-pointer shadow-lg shadow-rose-950/40 transition-colors"
        >
      ...
      <div className="bg-zinc-900 border border-zinc-700 rounded-xl overflow-hidden shadow-xl">
        <table className="w-full text-left text-sm text-zinc-200">
          ...
          <tbody className="divide-y divide-zinc-800">
            {wasteLogs.length === 0 ? (
              <tr>
                <td colSpan={6} className="p-8 text-center text-zinc-400 italic">No waste logs recorded yet.</td>
              </tr>
            ) : (
              wasteLogs.map(w => (
